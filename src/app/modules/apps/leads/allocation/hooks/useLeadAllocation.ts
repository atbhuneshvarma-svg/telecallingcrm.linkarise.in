...
export const useLeadAllocation = (): UseLeadAllocationReturn => {
  ...
  const fetchBulkAllocationData =
    async (): Promise<BulkAllocateGetResponse> => {
      setLoading(true);
      try {
  ...
  const bulkAllocateLeads = async (
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(
          `Successfully allocated leads to ${userIds.length} users`
        );
        await refreshData();
      } else {
  ...
  const deleteLeads = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_URL}/leadallocation/bulkDelete`,
        {
          lead_ids: selectedLeads, // ⬅ Send as array (correct way)
        }
      );

      if (response.data.success === true) {
        toast.success(response.data.message || "Leads deleted successfully");
      }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.duplicates && result.duplicates.length > 0) {
          toast.warning(
            `⚠️ Found ${result.duplicates.length} duplicate leads`,
            {
        ...
        if (
          ...
          toast.info(
            "📋 No new leads imported. Check duplicates/errors above."
          );
        }
      } else {
